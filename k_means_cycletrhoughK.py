...
#rdd_file=file.map(lambda row: parsePoint_fromString(row) ).cache()
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
for k in range(10):  #--->create loop over ks
	...
	iniCentroids = rdd_file.takeSample(False, k+1)
	...
	rdd_temp1=rdd_file.flatMap(lambda row:[(i, row) for i in range(k+1)]).cache()	# [ ( 0, (id1, x1, y1, z1, ...)),( 1, (id1, x1, y1, z1, ...),  (2, (p1),.....
	...
	# DISTANCE COMPUTATION
		...
		# print("rdd_temp1 :  ", rdd_temp1.collect())
...
# 	rdd_assign.saveAsTextFile(st_path)
	...
	outputFile = open("outputkmeans_K"+str(k)+"___" + nowsrt + ".csv", "w")
...
#OUTPUT CREATION:  output error through k
# now = datetime.datetime.now()
# nowsrt = now.strftime("%Y-%m-%d_%H%M")
outputCostOverK = open("costoverKs_" + nowsrt + ".csv", "w")
outputCostOverK.writelines(["%s\n" % item for item in list_cost_overK])
