...
# spark-submit --master yarn --executor-memory 7G  /homes/in304/bigdata/Assignment2/main_2withReduce.py
...
# CHANGE COMMENTed lines LINE TO TAKE ONLY A SUbSET OF POINTS:+++++++++++++++++++++++++++++++++++++++++++
rdd_file=file.map(lambda row:  parsePoint_fromString(row) )
# rdd_part=file.sample(False, 0.000003, 0)
# rdd_file=rdd_part.map(lambda row: parsePoint_fromString(row) ).cache()
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
mset = [list(x) for x in set(tuple(x) for x in iniCentroids[1:])]
while (len(iniCentroids[1:]) is not len(mset)):
	print("will take another sample1")
	# 			#take new samples until length is the same!
	iniCentroids = rdd_file.takeSample(False, k)
...
# 	sum_dist = rdd_closestCentroid.reduce(lambda a,b : a[1][1]+b[1][1])  #invalid index to scalar variable.
	sum_dist = rdd_closestCentroid.map(lambda row :  row[-1][-1]).reduce(lambda a,b: (a+b))
# 	print("SUM OF DISTANCESSSS TO COMP COST ", sum_dist)
	cost=sum_dist/count_points
...
# UPDATE CENTROIDS:
	rdd_temp = rdd_closestCentroid.map(lambda row: (row[1][0], row[0])).cache()
	...
	centroidsRDD = rdd_index_sum_counts.map(lambda row: (row[0], compute_newCentroids(row[-1][0],row[-1][-1])))
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#writes centroids table to file in local folder
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
print(list_cost)
#writes cost over iterations on local folder
