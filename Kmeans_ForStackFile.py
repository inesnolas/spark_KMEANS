...
file = spark.read.option("header","true").csv("/user/group-AI/output_user_clean.csv").rdd.map(tuple)
...
rdd_part=file.sample(False, 0.0003, 0)
...
n_max_clusters=5
print("Not yet... later to add... Going to loop from K=2 to K=", n_max_clusters)
...
iniCentroids = rdd_part_parsed.takeSample(False,k) 
...
#   if itr in arange(0, 10000, 10):
    print("K=",k,"cost=", cost, "delta_cost=", delta_cost)
